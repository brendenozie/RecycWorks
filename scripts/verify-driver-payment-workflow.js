...
const mongoUri = process.env.MONGODB_URI || "mongodb://localhost:27017/RecycWorks";
const jwtSecret = process.env.JWT_SECRET || "default-jwt-secret-key-change-in-production-trading-app";
...
async function runTest() {
  ...
  const foUser = {
    _id: new ObjectId(),
    email: "sarah.fo.drivertest@recycworks.com",
    firstName: "Sarah",
  ...
  const driverUser = {
    _id: new ObjectId(),
    email: "james.driver.drivertest@recycworks.com",
    firstName: "James",
    lastName: "Kamau",
    role: "driver",
  ...
  const adminUser = {
    _id: new ObjectId(),
    email: "alice.admin.drivertest@recycworks.com",
    firstName: "Alice",
    lastName: "Admin",
    role: "admin",
    status: "active",
  ...
  const supplierUser = {
    _id: new ObjectId(),
    email: "kip.supplier.drivertest@recycworks.com",
    firstName: "Kip",
  ...
  const foToken = signUserToken({ userId: foUser._id.toString(), email: foUser.email, role: "field_officer" });
  const driverToken = signUserToken({ userId: driverUser._id.toString(), email: driverUser.email, role: "driver" });
  const adminToken = signUserToken({ userId: adminUser._id.toString(), email: adminUser.email, role: "admin", isAdmin: true });
  ...
  try {
    ...
    const foundReady = dispatchListData.readyCollections.find((c) => c.id === collectionId);
